...
export default function Carousel() {
  ...
  const scrollToIndex = (index) => {
    ...
    if (itemsPerView === 1) {
      ...
      carouselRef.current.style.transform = `translateX(-${index * (itemWidth + gap)}px)`;
    }
  };
                ...
                <div className="relative group/card cursor-pointer">
                  ...
                  <div className="absolute inset-0 bg-black/0 group-hover/card:bg-black/40 transition-all duration-300 rounded-lg flex items-end p-4 opacity-0 group-hover/card:opacity-100">
                    <div className="text-white">
                      <h3 className="text-base sm:text-lg font-bold">{movie.title}</h3>
                      <p className="text-xs sm:text-sm text-gray-300">{movie.category}</p>
                    </div>
                  </div>
                </div>
              </div>
